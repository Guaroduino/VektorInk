...
export class LegacyInkTool implements ITool {
  ...
  constructor(meshManager: MeshManager, gui: GUI) {
    ...
    legacyFolder.add(this.strokeSettings, 'feather', 0.6, 1.0, 0.01).name('Feather').onChange((v: number) => {
      this.meshManager.setFeather(v);
    });
  ...
  public onPointerDown(event: VektorPointerEvent, targetLayer: Container): void {
    ...
    // Apply initial feather
    this.meshManager.setFeather(this.strokeSettings.feather);
  }
